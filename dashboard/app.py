...
def prepare_endpoint_display_data(data):
    ...
    # Format status text
    display_data['status_text'] = str(
        data['status_code']) if data['status_code'] else 'N/A'
...
def index(request):
    ...
    endpoint_data.sort(key=lambda x: (
        x['days_left'] is None, x['days_left'] if x['days_left'] is not None else float('inf')))

    # Prepare display data
    display_endpoints = [prepare_endpoint_display_data(
        data) for data in endpoint_data]
